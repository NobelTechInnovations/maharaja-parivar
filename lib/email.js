...
export async function sendEmail({ to, subject, html, text }) {
  const from = EMAIL_FROM || "Maharaja Parivar <no-reply@maharajaparivar.com>";
...
const BRAND_HEADER = `
  <div style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;max-width:480px;margin:0 auto;">
    <div style="font-family:Georgia,serif;font-size:20px;color:#211c14;margin-bottom:20px;">Maharaja Parivar</div>
`;
const BRAND_FOOTER = `
    <p style="margin-top:28px;font-size:12px;color:#6c6252;">
      Maharaja Parivar — an independent alumni community for University Maharaja's College, Jaipur.
    </p>
  </div>
`;
...
export function sendWelcomeEmail(user) {
  return sendEmail({
    to: user.email,
    subject: "Welcome to Maharaja Parivar",
    text: `Hi ${user.name}, thanks for joining Maharaja Parivar. Your registration is being reviewed by our team — you'll get an email once your profile is verified.`,
    html: `${BRAND_HEADER}
      <p>Hi ${user.name},</p>
      <p>Thanks for joining <strong>Maharaja Parivar</strong>. Your registration is now waiting on our team's review — you'll get an email as soon as your profile is verified and visible to other Maharaja Fellow.</p>
      ${BRAND_FOOTER}`,
  });
}

export function sendAccountApprovedEmail(user) {
  const url = process.env.NEXT_PUBLIC_APP_URL || "https://maharajaparivar.com";
  return sendEmail({
    to: user.email,
    subject: "You're verified — welcome to Maharaja Parivar",
    text: `Hi ${user.name}, your account has been approved. Log in to complete your profile and start finding fellow Maharaja Fellow: ${url}/login`,
    html: `${BRAND_HEADER}
      <p>Hi ${user.name},</p>
...
export function sendPasswordResetEmail(user, token) {
  ...
  return sendEmail({
    to: user.email,
    subject: "Reset your Maharaja Parivar password",
    text: `Reset your password: ${link} (expires in 1 hour). If you didn't request this, ignore this email.`,
    html: `${BRAND_HEADER}
      <p>Hi ${user.name},</p>
      <p>We got a request to reset your Maharaja Parivar password. This link expires in 1 hour.</p>
